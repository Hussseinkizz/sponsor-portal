...
function html(strings, ...values) {
  ...
  function evalValue(value) {
    if (
      typeof value === 'object' &&
      (value == null ? void 0 : value.type) === 'LIST'
    ) {
...
function StateRadio(options = {}) {
  ...
  const usePlugin = (channelName, pluginName) => {
    ...
    if (plugin) {
      ...
      exposedMethods.forEach((item) => {
        channels2[channelName][item.name] = (...args) =>
          item.method(channels2[channelName].state, ...args);
      });
  ...
  const notifySubscribers = (channelName) => {
    channels2[channelName].subscribers.forEach((subscriber) =>
      subscriber(channels2[channelName].state)
    );
  };
  ...
  const removeChannel = (channelName) => {
    if (channels2[channelName]) {
      const updatedChannels = Object.keys(channels2)
...
function Router(config = {}) {
  ...
  const navigate = (urlPath, options = {}) => {
    ...
    const renderComponent =
      options.component || (route == null ? void 0 : route.component);
    if (renderComponent) {
